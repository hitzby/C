...
void freeBinnode(Binnode *bn) {
	if (bn == NULL) return;
	freeBinnode(bn->child);
	freeBinnode(bn->sibling);
	free(bn);
}
...
int main(void) {
	...
	Binqueue *bq = (Binqueue *)malloc(sizeof(Binqueue));//在这里开始创造二项队列，在这个队列中储存森林的地址
	bq->current_size = n;
	...
	//释放内存
	for (int i = 0; i < n; i++) {
		Bintree *bt = bq->queue[i];
		for (int j = 0; j < MAX_RANK; j++) {
			if (bt->trees[j] != NULL) {
				// 递归释放二项树节点
				freeBinnode(bt->trees[j]);
			}
		}
		free(bt->trees);
		free(bt);
